...
for i = 1:numImages
    ...
    if ~isempty(bboxFace)
        ...
        if ~isempty(bbox)
            ...
            % If no mouth, process entire image
        ...
        % Create a new mask with only the 3 largest components
        ...
        % Create a mask for pixels below the skin region
        ...
        % Redefine thresholds to add skin in
        ...
        if chosenHue < 0.135 || chosenHue > 0.45
            ...
            % Morphological operations on green regions
            se = strel('disk', 5);
            ...
            % Remove green regions from the final mask
        ...
        imshow(finalImg); 
        title(['Result ' num2str(i)], 'FontSize', 12); 
